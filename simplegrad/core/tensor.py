...
def graph_group(name: str):
    ...
    Used internally by the ``compound_op`` decorator. Every Tensor created
    while this context is active receives a ``group`` attribute of
    ``(name, unique_id)``, which the graph renderer uses to draw a labelled
    rectangle around all nodes belonging to the same compound operation call.
...
def compound_op(func):
    ...
    Apply to any public function that builds its output by calling other
    simplegrad ops rather than calling numpy directly. When the function is
    called, all Tensors created during its execution are tagged with a shared
    group identifier. The computation graph renderer draws a black-border
    rectangle around those nodes, labelled with the function name.

    Each call to the decorated function creates a new unique group, so two
    calls to ``softmax`` produce two separate rectangles in the graph.

    Args:
        func: The compound function to decorate. Its ``__name__`` is used as
            the cluster label.
...
class Tensor:
    ...
    def __init__(
        ...
        self.group: tuple[str, int] | None = _CURRENT_GROUP
